...
const packsInStandAlone = {
  server: "git+https://github.com/zaion-network/server#v0.0.1",
...
const PATHSMAP = new Map().set(true, fromRoot).set(false, fromScript);

export const app = new App("zaion", "@zaionstate", PATHSMAP, PACKSMAP);
...
const monorepoChecker = ({ json, reponame }) => json.name === reponame;
...
const getter = ({ parsed, scope }) => {
  ...
  if ("peerDependencies" in parsed) {
    const keys = Object.keys(parsed.peerDependencies[k]);
    keys.forEach(k => {
      deps.push(["peerDependencies", k, parsed.peerDependencies[k]]);
    });
  }
  return deps
...
const adapter = ({ deps, map, target }) => deps.map(depsMapper(map, target));
