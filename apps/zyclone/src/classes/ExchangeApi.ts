...
import { Binance as B } from "./ExhangeApi/Binance";
...
export namespace ExchangeApi {
  ...
  export interface cancelOrderProps extends _object {
    symbol: string;
    orderId: number;
    timestamp?: number;
    signature?: string;
  ...
  interface ExchangeApiCtorProps {
    source: string;
  }
  export enum sources {
  ...
  export interface ExchangeApi<Price, Candle, Order, OrderDetails> {
    source: string;
    getTickerPrice(props: getTickerPriceProps): Promise<Price>;
    getTickerCandles(props: getTickerCandlesArgs): Promise<Candle>;
    createOrder(props: createOrderProps): Promise<Order>;
    cancelOrder(props: cancelOrderProps): Promise<Order>;
    getOrder(id: number): Promise<Order>;
    getOrderDetails(id: number): Promise<OrderDetails>;
    getOrders(props: getOrdersProps): Promise<Order[]>;
    getOrdersDetails(props: getOrdersDetailsProps): Promise<OrderDetails[]>;
    timestamp(): number;
  }
  export class ExchangeApi<Price, Candle, Order, OrderDetails>
    implements ExchangeApi<Price, Candle, Order, OrderDetails>
  {
    protected API_KEY: string;
    protected API_SEC: string;
    timestamp = () => Date.now();
    constructor(API_KEY: string, API_SEC: string, props: ExchangeApiCtorProps) {
  ...
  export namespace Binance {
    ...
    export interface BinanceApi
      extends ExchangeApi<Price, Candle, Order, OrderDetails> {}
    ...
    > {
      constructor(API_KEY: string, API_SEC: string) {
        super(API_KEY, API_SEC, { source: sources.binance });
      }

      requester<T, H = undefined>(
        method: string
      ): (props: getterProps<H>) => Promise<T> {
        return async function (props) {
          return await httpRequest<T>({
            hostname: sources.binance,
            path: props.path,
            method: method,
      ...
      headers_1: _object = { "X-MBX-APIKEY": this.API_KEY };

      async getter<T, H = undefined>(props: getterProps<H>): Promise<T> {
        return this.requester<T, H>(httpRequest.requestTypes.get)(props);
      }
      async poster<T, H = undefined>(props: getterProps<H>): Promise<T> {
        return this.requester<T, H>(httpRequest.requestTypes.post)(props);
      }
      async deleter<T, H = undefined>(props: getterProps<H>): Promise<T> {
        return this.requester<T, H>(httpRequest.requestTypes.delete)(props);
      }

      async getTickerPrice(props: getTickerPriceProps): Promise<Price> {
      ...
      async getTickerCandles(props: getTickerCandlesArgs): Promise<Candle> {
      ...
      async getOrders(props: getOrdersProps): Promise<Order[]> {
      ...
      async cancelOrder(props: cancelOrderProps): Promise<any> {
